...
2. **Fail-closed mapping** — missing `observedAt` / `universeDefinition` → `schema_error`; owner counts null stay null (UI `—`); ratio null stays null; no browser `Date.now()` synthesis.
