...
| F3 | Pre-existing tree contamination blocks `npm run harness:doctor` / secrets integrity for **this** audit run: (1) implementer task inputs use absolute external filesystem paths (doctor requires repo-relative); (2) false-positive `INLINE_API_CREDENTIAL` on a local variable assignment whose identifier matched the scanner pattern (not a credential value); (3) synthetic test contained a static PEM private-key block header. None of these are live secrets; all are outside the auditor write set. Follow-up repair `GMGN-CLI-PROXY-TRANSPORT-ROOT-CAUSE-REPAIR-001` clears these doctor blockers. | P1 (harness gate) | Forces formal harness finish to `GREEN_WITH_ADVISORY` rather than strict `GREEN` |
