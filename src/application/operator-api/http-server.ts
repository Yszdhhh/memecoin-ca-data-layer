...
async function handle(
  ...
  try {
    ...
    // --- Watchlist + local alerts (research notify only) ---
    if (method === "GET" && url.pathname === "/api/v1/watchlist") {
      return json(res, 200, { items: offline.watchlist.listWatches() });
    }
    if (method === "POST" && url.pathname === "/api/v1/watchlist") {
      ...
      try {
        const item = offline.watchlist.addWatch({
          kind: body.kind === "address" ? "address" : "ca",
          subject: String(body.subject ?? ""),
          ...(body.label !== undefined ? { label: body.label } : {}),
          ...(body.cooldownMinutes !== undefined ? { cooldownMinutes: body.cooldownMinutes } : {}),
        });
        // Evaluate once if CA card known
        if (item.kind === "ca") {
          const scan = offline.getCaScan(item.subject);
          if (scan?.analysis) {
            const a = scan.analysis as {
              dataQuality?: { accountingEligible?: boolean | null; concentrationEligible?: boolean | null };
              market?: { liquidityUsd?: number | null };
              addressHits?: unknown[];
              identity?: { mintAuthority?: string | null };
            };
            offline.watchlist.evaluateCaSnapshot(item.subject, {
              accountingEligible: a.dataQuality?.accountingEligible ?? null,
              concentrationEligible: a.dataQuality?.concentrationEligible ?? null,
              liquidityUsd: a.market?.liquidityUsd ?? null,
              addressHitCount: a.addressHits?.length ?? 0,
              mintAuthorityPresent: a.identity?.mintAuthority != null,
            });
          }
        }
        return json(res, 201, item);
      } catch (e) {
        return json(res, 400, { error: e instanceof Error ? e.message : "watch_failed" });
      }
    }
    const watchEnMatch = url.pathname.match(/^\/api\/v1\/watchlist\/([^/]+)\/enabled$/);
    if (method === "POST" && watchEnMatch) {
      const body = (await readJson(req)) as { enabled?: boolean };
      const w = offline.watchlist.setEnabled(decodeURIComponent(watchEnMatch[1]!), body.enabled === true);
      if (!w) return json(res, 404, { error: "watch_not_found" });
      return json(res, 200, w);
    }
    if (method === "GET" && url.pathname === "/api/v1/alerts") {
      const unreadOnly = url.searchParams.get("unread") === "1";
      return json(res, 200, {
        items: offline.watchlist.listAlerts({ unreadOnly }),
        unreadCount: offline.watchlist.listAlerts({ unreadOnly: true }).length,
      });
    }
    if (method === "POST" && url.pathname === "/api/v1/alerts/read-all") {
      return json(res, 200, { marked: offline.watchlist.markAllRead() });
    }
    const alertReadMatch = url.pathname.match(/^\/api\/v1\/alerts\/([^/]+)\/read$/);
    if (method === "POST" && alertReadMatch) {
      const a = offline.watchlist.markRead(decodeURIComponent(alertReadMatch[1]!));
      if (!a) return json(res, 404, { error: "alert_not_found" });
      return json(res, 200, a);
    }

    // --- Schedules enable toggle ---
    const schedEnMatch = url.pathname.match(/^\/api\/v1\/schedules\/([^/]+)\/enabled$/);
    if (method === "POST" && schedEnMatch) {
      const body = (await readJson(req)) as { enabled?: boolean };
      const s = offline.schedules.setEnabled(decodeURIComponent(schedEnMatch[1]!), body.enabled === true);
      if (!s) return json(res, 404, { error: "schedule_not_found" });
